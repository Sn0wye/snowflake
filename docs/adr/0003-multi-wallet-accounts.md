...
- **Exactly one Wallet per (Account, currency)** is enforced by a DB `UNIQUE(account_id, currency)` constraint — not app checks alone — because lazy creation has no existing row to lock and concurrent first-deposits in the same currency would otherwise insert duplicates. Lazy creation upserts (`ON CONFLICT DO NOTHING`) then re-fetches the row `FOR UPDATE`, so the losing racer re-reads the winner's Wallet and the operation still completes. Currency is normalized to uppercase ISO 4217 and validated against the allowlist at the boundary, so the constraint is meaningful (`usd` and `USD` can never become two Wallets).
